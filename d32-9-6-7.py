# DAY 32: REGULARIZATION AND GENERALIZATION

# le plan (written tonight for tomorrow me):
# separate again into training/validation/testing datasets
# probably split 7000/1500/1500
# experiment with massive nns, like [2,64,64,64,5] to see if overfitting happens (god this will take forever to run)
# experiment with smaller nns, like [2,4,5] to see if that's still fine
# nn will hopefully (not hopefully but hopefully) start learning noise
# to fix: L2 organization
# penalizing network for having massive weights
# after creating the weight gradients do something like weight_gradient += lambda_ * weights[l]. don't do same for biases
# this makes the gradient larger for larger weights and vice versa, etc...
# btw discouraging large weights makes less complicated solutions
...
def neural_network_multiple_hidden_layers(layer_sizes, iterations, X, y, learning_rate, lambda_):
    ...
    for i in range(iterations):
        ...
        for k in range(len(layer_sizes)-2):
            ...
            weight_gradient += 2 * lambda_ * weights[-k-1]
...
# multiple layers
weights, biases = neural_network_multiple_hidden_layers([2,8,8,5], 2500, X_normalized, grade, 0.1, 0.01)
...
"""
NOTES:
Starting without regularization, with an extremely large NN
[2,64,64,64,5]: produced very low loss, and surprisingly didn't overfit too much
Increasing the noise from -5,6 to -10,11
After increasing to -10,11
[2,64,64,64,5]: Loss went up from earlier and accuracy went down, so I don't think
this was an overfitting problem, just a noise problem
[2,8,8,5]: Smaller, just want to see how it will do: Similar results, about 0.8 accuracy in validation
and more chaotic confusion matrix, just a noise problem
I think I'd be able to see overfitting more if the dataset was smaller, so decreasing it to 1000.
[2,8,8,5]: Still works about the same with this smaller set.
[2,64,64,64,5]: Accuracy became worse (went down to 75% ish), but not horribly so, 
and the confusion matrix is still decently logical
[2,128,128,128,128,5]: Since NUM_STUDENTS is pretty small this won't take forever: it
still isn't doing bad, not the type of overfitting i expected to see
[2,512,256,128,64,5]: Still not so bad. I don't know how to get it to overfit
Drastic change, NUM_STUDENTS is getting changed to 100.
Now it's too small, so I'm really not sure how to get a good overfitting example. Changing back to
1000 and then increasing noise to -20,21
Loss is high and accuracy is lower, this isn't really overfitting
I guess with these big NNs, the training accuracy is a bit bigger than the validation accuracy
like 75% to 65% but that's not nearly a big a jump as I thought.
However, with the bigger dataset, training accuracy ~ validation accuracy, so maybe this is a
bit of an example of overfitting
