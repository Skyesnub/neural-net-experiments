...
print(data.head())
print(repr(data.iloc[0, 0]))

print(data.shape)
print(data.dtypes)

y = data.iloc[:, -1].to_numpy() # each column, the first value is the class that it was given (in this case rating from 0-10)
X = data.iloc[:, 1:].to_numpy() # the values fron 2 onwards are the data that contributed to the taste

indices = np.arange(len(X))
np.random.shuffle(indices)

X = X[indices]
y = y[indices] # shuffling dataset

print(np.unique(y))

train_size = len(X) // 10 * 8

testing_X = X[train_size:]
testing_y = y[train_size:]

y = y[:train_size]
X = X[:train_size]

print(testing_X.shape, "testing x shape")
...
print("Actual:", np.bincount(y, minlength=11))

weights, biases = neural_network_multiple_hidden_layers([11,16,8,11], 5000, X_normalized, y, 0.2, 0.01, 0)
...
mult_layer_accuracy, mult_confusion = test_my_neural_network(weights, biases, testing_X_normalized, testing_y, 11)
