# DAY 24: VALIDATION SETS AND HYPERPARAMETER SEARCHING

import numpy as np
#np.random.seed(42) # removing the randomization for now (helps with validation) and having the exact same students
#removing randomness for now
...
X = np.column_stack((hours_studied[:NUM_STUDENTS//10*7], hours_slept[:NUM_STUDENTS//10*7])) # this will be training X
testing_X = np.column_stack((hours_studied[NUM_STUDENTS//10*7:NUM_STUDENTS//20*17], hours_slept[NUM_STUDENTS//10*7:NUM_STUDENTS//20*17]))
validation_X = np.column_stack((hours_studied[NUM_STUDENTS//20*17:], hours_slept[NUM_STUDENTS//20*17:]))
...
validation_distance = np.abs(
    validation_X[:, 0] + validation_X[:, 1] - 12
) # creating the validation scores with the exact same rules as before

validation_scores = (
    100
    - validation_distance**2 * 5
    + np.random.randint(-5, 6, size=len(validation_X))
)

validation_grade = np.zeros(len(validation_X), dtype=int)
...
validation_grade[validation_scores < 50] = 0 # finalizing the validation scores with 43210 for the grades
...
NUM_NEURONS_LIST = [4,8,12,16]
LEARNING_RATES_LIST = [0.1,0.05,0.5]

results = {}
models = {}


for neuron_index, NUM_NEURONS in enumerate(NUM_NEURONS_LIST):
    for lr_index, learning_rate in enumerate(LEARNING_RATES_LIST):
        ...
        for i in range(5000):
            ...
            #if i % 100 == 0:
                #print(i, loss) # commented out for efficiency (print is expensive)

        validation_hidden_logits = (
            validation_X_normalized @ hidden_weights + hidden_bias
        )
        validation_hidden_activation = relu(validation_hidden_logits)
        validation_output_logits = (
            validation_hidden_activation @ output_weights + output_bias
        )
        validation_probabilities = softmax(validation_output_logits)
        validation_predictions = np.argmax(
            validation_probabilities,
            axis=1
        )
        validation_accuracy = np.mean(
            validation_predictions == validation_grade # calculating the accuracy the exact same way i did for the test set
        )
        print(
            NUM_NEURONS,
            learning_rate,
            validation_accuracy
        ) # printing the results that i'm getting out of this experiment

        results[(NUM_NEURONS, learning_rate)] = validation_accuracy

        models[(NUM_NEURONS, learning_rate)] = {
            "hidden_weights": hidden_weights.copy(),
            "hidden_bias": hidden_bias.copy(),
            "output_weights": output_weights.copy(),
            "output_bias": output_bias.copy()
        }

best_configuration = max(results, key=results.get)

print("Best configuration:", best_configuration)
print("Best validation accuracy:", results[best_configuration])

best_model = models[best_configuration] # getting the best model

hidden_weights = best_model["hidden_weights"]
hidden_bias = best_model["hidden_bias"]
output_weights = best_model["output_weights"]
output_bias = best_model["output_bias"] # getting the best models weights and biases, then updating the testing weights and biases
# that are used on the test set
