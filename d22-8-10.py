...
import matplotlib.pyplot as plt # plottng the scatterplot

plt.scatter(
    X[:, 0],              # hours studied
    X[:, 1],              # hours slept
    c=grade               # color based on grade
)
...
print(hidden_weights[0]) # checking individual neurons here
print(hidden_bias[0])

neuron = hidden_activation[:, 0]
print(np.mean(neuron > 0))
...
actual_jitter = testing_grade + np.random.uniform(-0.15, 0.15, len(testing_grade))
predicted_jitter = testing_predictions + np.random.uniform(-0.15, 0.15, len(testing_predictions))
...
plt.yticks([0, 1, 2, 3, 4], ["F", "D", "C", "B", "A"])

plt.grid()
plt.show() # the jitter scatterplot will also allow me to see the density of the students and stuff
...
plt.show() # imshow makes a heatmap which is quite useful for my confusion matrix

# DECISION REGION PLOT

# Create a grid of possible student inputs
study_range = np.linspace(2, 10, 300)
sleep_range = np.linspace(2, 10, 300)

study_grid, sleep_grid = np.meshgrid(study_range, sleep_range)

# Turn the grid into a list of (study hours, sleep hours) pairs
grid_points = np.column_stack((
    study_grid.ravel(),
    sleep_grid.ravel()
))

# Normalize using the SAME mean/std from the training data
grid_normalized = (grid_points - mean) / std

# Run grid through the trained neural network
grid_hidden_logits = grid_normalized @ hidden_weights + hidden_bias
...
# Reshape predictions back into the 300x300 grid
grid_predictions = grid_predictions.reshape(study_grid.shape)
...
# Plot the actual training students on top
plt.scatter(
    X[:, 0],
    X[:, 1],
    c=grade,
