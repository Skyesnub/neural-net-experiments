...
def batched_nn(layer_sizes, X, y, learning_rate, batch_size, num_epochs, use_He=True):
    weights = []
    biases = []
    loss_lst = []
    ...
    for epoch in range(num_epochs):
        ...
        for start in range(0, n, batch_size): # will go from 0 to the end of the list in increments of batch size, logically makes sense

            batch_indices = indices[start:start + batch_size]

            X_batch = X[batch_indices]
            y_batch = y[batch_indices]
            y_onehot_batch = y_onehot[batch_indices]
            ...
            # OUTPUT
            output_error = probabilities - y_onehot_batch
            error = output_error
            ...
            weight_gradients[0] = (1/len(X_batch)) * X_batch.T @ error
            ...
            # for each student
        loss_lst.append(np.mean(epoch_losses))
...
# multiple layers
weights, biases, loss_lst = batched_nn([2,8,8,5], X_normalized, grade, 0.1, 64, 50, use_He=True)
